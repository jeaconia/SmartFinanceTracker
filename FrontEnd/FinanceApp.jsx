...
export default function App() {
  ...
  return (
    <div style={{ display: "flex", minHeight: "100vh", background: "#F0EDD8", fontFamily: "'DM Sans',sans-serif", fontSize: 14 }}>
      ...
      <Sidebar active={page} onChange={(p) => { setPage(p); setShowNotif(false); }} />
...
// ---------------- Pages (Dashboard, Grafik, Budgeting, Catatan)
        ...
        <div style={{ background: "white", borderRadius: 18, padding: "18px 20px", border: "2px solid #C8D4A0", position: "relative", overflow: "hidden" }}>
          ...
          {label ? (
            <>
              ...
              {label.traits?.slice(0, 2).map((t, i) => (
                <div key={i} style={{ fontSize: 11, color: "#666", marginBottom: 2 }}>• {t}</div>
              ))}
              {label.confidence && (
                <div style={{ fontSize: 10, color: "#bbb", marginTop: 6 }}>Akurasi AI: {Math.round(label.confidence * 100)}%</div>
              )}
            </>
          ...
          {pie.map((d) => (
            <div key={d.name} style={{ display: "flex", justifyContent: "space-between", fontSize: 10, padding: "2px 0", borderBottom: "1px solid rgba(255,255,255,0.12)" }}>
              <span style={{ display: "flex", alignItems: "center", gap: 4 }}>
                <span style={{ width: 6, height: 6, borderRadius: "50%", background: d.color, display: "inline-block", border: "1px solid rgba(255,255,255,.3)" }} />
                {d.name}
              </span>
    ...
    <div style={{ padding: "20px 24px", animation: "fadeIn .3s ease" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 220px", gap: 12, marginBottom: 12 }}>
        ...
        <div style={{ background: "white", borderRadius: 18, padding: 16 }}>
          ...
          <div style={{ display: "flex", justifyContent: "center", marginBottom: 10 }}>
            <PieChart data={pieSpend.map(d => ({ ...d, color: d.color + "99" }))} size={110} />
          </div>
          {pieSpend.map(d => (
            <div key={d.name} style={{ display: "flex", justifyContent: "space-between", fontSize: 11, padding: "2px 0", borderBottom: "1px solid #fafafa" }}>
              <span style={{ display: "flex", alignItems: "center", gap: 5 }}>
                <span style={{ width: 7, height: 7, borderRadius: 2, background: d.color + "99", display: "inline-block" }} />
                {d.name}
              </span>
        ...
        {pred && (
          <div style={{ background: "linear-gradient(135deg,#3A5C28,#5a6b3a)", borderRadius: 18, padding: 16, color: "white" }}>
            ...
            {pred.context?.change_pct != null && (
              <div style={{ fontSize: 11, opacity: 0.8, marginTop: 4, marginBottom: 12 }}>{pred.context.change_pct > 0 ? "▲" : "▼"} {Math.abs(pred.context.change_pct)}% dari bulan lalu</div>
            )}
            {pred.context?.based_on_months && (
              <div style={{ fontSize: 11, opacity: 0.65, fontStyle: "italic" }}>Prediksi berdasarkan {pred.context.based_on_months} bulan terakhir</div>
            )}
          </div>
        )}
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 250px", gap: 16 }}>
        ...
        <div>
          <div style={{ background: "#2D4A1E", borderRadius: 18, padding: 16, color: "white", marginBottom: 12 }}>
            ...
            {recs?.basedOnCity && (
              <div style={{ fontSize: 10, opacity: 0.55, marginTop: 8 }}>Berdasarkan UMR {recs.basedOnCity}</div>
            )}
          </div>
...
function Catatan() {
  ...
  const reloadTx = useCallback(() => {
    API.listTransactions({ type: tab === "rutin" ? undefined : tab, month }).then(r => { setTxs(r.data || []); setPagination(r.pagination); }).catch(() => {});
  }, [tab, month]);
  ...
  const handleSaveRec = async () => { if (!recForm.name || !recForm.amount || !recForm.next_due_date) return; await API.createRecurring({ ...recForm, amount: Number(recForm.amount) }).catch(() => {}); setShowRecForm(false); API.listRecurring(true).then(setRecurring).catch(() => {}); };
        ...
        {tab !== "rutin" && (
          <>
            ...
            {pagination && pagination.totalPages > 1 && (
              <div style={{ textAlign: "center", padding: "12px 0", fontSize: 12, color: "#999" }}>Hal 1 dari {pagination.totalPages} · {pagination.total} transaksi</div>
            )}
          </>
        )}
